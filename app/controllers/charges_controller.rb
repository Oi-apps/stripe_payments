class ChargesController < ApplicationController
	...
	def create
	  ...
	  # 	card: params[:stripeToken], amount: product.price_in_cents, description: charge.description, currency: charge.currency,
	  # 	customer_id: customer.id, product_id: product.id, uuid: SecureRandom.uuid)


	    purchase = Purchase.create(email: params[:stripeEmail], 
	  		card: params[:stripeToken], amount: product.price_in_cents, description: product.full_description, 
